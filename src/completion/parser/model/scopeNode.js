...
/*export default class extractData {
	...
	extractClassArray(classArray, scope = "global", scopetype = "main") {
		classArray.forEach(e => {
			...
			e.parameters.forEach((p, i) => {
				let comma = ''
				if ((e.parameters.length - 1) - i != 0) comma = ','
				parameter += `${p.name}:${p.type}${comma}`
			})
			const code = `${name}(${parameter})`
			this.data.push({ scope: Scope, scopeType: scopeType, name: name, type: type, code: code,parameters:e.parameters })
	...
	extractFunctionArray(functionArray, scope = "global", scopetype = "main", type = "function") {
		for(let e of functionArray){
			...
			const isReturn= (returnType != "Void")? true : false
			e.parameters.forEach((p, i) => {
				let comma = ''
				if ((e.parameters.length - 1) - i != 0) comma = ','
				parameter += `${p.name}:${p.type}${comma}`
			...
			let derive = ""
			if (scopetype == "class") derive = `${scope}.`
			const code = `${derive}${name}(${parameter}): ${returnType}`
			this.data.push({ scope: scope, scopeType: scopetype, name: name, type: type, code: code ,parameters:e.parameters,isReturn:isReturn})
			this.extractClassArray(e.body.classDeclaration, name, type)
	...
	extractVariableArray(variableArray,scope="global",scopetype="main"){
		for(let e of variableArray){
			const name=e.name
			const type=e.type?e.type:"Any"
			const isMutable=e.isMutable
			const code = (scopetype=="class")? `${scope}.${name}` : `${name}`
			this.data.push({scope:scope,scopeType:scopetype,name:name,type:type,code:code})
		}
	}
	extractImportArray(importArray,scope="global",scopetype="main"){
		for ( let e of importArray){
			e.importKeyword.forEach(i=>{
			this.data.push({scope:scope,scopeType:scopetype,name:i,type:"local"})
			})
			if(e.isAlias){
				
				this.data.push({scope:scope,scopeType:scopetype,name:e.importAlias,type:"local"})
			}
		}
	}
